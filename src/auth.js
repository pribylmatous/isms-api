...
export function createAuth(db) {
  ...
  function registerAuthRoutes(app) {
    ...
    app.get('/api/auth/me', async (req, res) => {
      const user = await userForToken(parseCookies(req.headers.cookie)[COOKIE]);
  ...
  async function upsertSsoUser({ oid, name, email, role }) {
    const existing = await db.prepare('SELECT * FROM users WHERE entra_oid = ?').get(oid);
    if (existing) {
      if (!existing.active) return null;
      await db.prepare('UPDATE users SET name = ?, email = ?, role = ? WHERE id = ?').run(name, email, role, existing.id);
      return db.prepare('SELECT * FROM users WHERE id = ?').get(existing.id);
    }
    const username = `entra:${oid}`;
    const inserted = await db.prepare(
      'INSERT INTO users (username, name, email, role, password_hash, entra_oid, created_at) VALUES (?, ?, ?, ?, ?, ?, ?) RETURNING *',
    ).get(username, name, email, role, hashPassword(crypto.randomBytes(32).toString('hex')), oid, new Date().toISOString());
    return inserted;
  }
